...
def train(env):
    ...
    # model = QRDQN("MultiInputPolicy", env, learning_rate=3e-4, batch_size=64, policy_kwargs=policy_kwargs, verbose=1)
    # model = TQC("MultiInputPolicy", env, top_quantiles_to_drop_per_net=2, verbose=1)
    model = PPO("MultiInputPolicy", env, verbose=1, ent_coef=0.01, policy_kwargs=policy_kwargs, tensorboard_log=log_dir)
    print(model.policy)
    checkpoint_callback = CheckpointCallback(
        save_freq=100000, save_path=log_dir, name_prefix="rl_model_v1"
    )
